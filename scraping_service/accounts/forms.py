...
class UserLoginForm(forms.Form):
    ...
    def clean(self, *args, **kwargs):
        ...
        if email and password:
            ...
            if not user:
                raise forms.ValidationError('Даннй аккаунт отключен')
        return super(UserLoginForm, self).clean(*args, **kwargs)


class UserRegistrationForm(forms.ModelForm):
    email = forms.CharField(label='Введите адрес электронной почты', widget=forms.EmailInput(attrs={'class' : 'form-control'}))
...
class ContactForm(forms.Form):
    ...
    language = forms.CharField(required=True, widget=forms.TextInput(attrs={'class' : 'form-control'}),
                               label='Специальность')
    email = forms.CharField(label='Введите адрес электронной почты', required=True,
